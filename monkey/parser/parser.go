...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefix(token.INT, p.parseIntegerLiteral) // we also include integers
	p.registerPrefix(token.BANG, p.parsePrefixExpression)
	p.registerPrefix(token.MINUS, p.parsePrefixExpression)

	// associate infix parse functions
...
// This function is the heart of Pratt parser.
// In parseExpression, for now it  handles the lowest possible precedence only
// more indepth explanation in ch-2.8. page#69
...
//											*
//	 left_operand   operator (current token)   right_operand (next token)
